...
集中维护 NL2SQL、意图识别、数据分析、图表生成和联网搜索相关提示词。
"""
...
NL2SQL_EXAMPLES = [
    {
        "question": "刷卡金平均金额最高的活动是哪个？",
        "sql": """SELECT ac.activity_name, AVG(br.bonus_amount) AS avg_bonus
FROM activity_config ac
JOIN cash_bonus_record br ON ac.activity_id = br.activity_id
GROUP BY ac.activity_id, ac.activity_name
ORDER BY avg_bonus DESC
LIMIT 1"""
    },
    {
        "question": "单笔刷卡金超过20元的记录有多少条？",
        "sql": """SELECT COUNT(*) AS high_bonus_count
FROM cash_bonus_record
WHERE bonus_amount > 20"""
    },
    {
        "question": "餐饮类交易产生刷卡金最多的3位用户是谁？",
        "sql": """SELECT ui.user_name, ui.phone, SUM(br.bonus_amount) AS total_bonus
FROM user_info ui
JOIN card_transaction ct ON ui.user_id = ct.user_id
JOIN cash_bonus_record br ON ct.trans_id = br.trans_id
WHERE ct.merchant_type = '餐饮'
GROUP BY ui.user_id, ui.user_name, ui.phone
...
2. sql_only - 查询【信用卡系统数据库】中的具体数据，不需要外部信息，也不需要深度分析
   示例：一共有多少笔交易、张三的刷卡金有多少、餐饮类交易有哪些

3. analysis_only - 只分析已有数据，不需要新查询
   示例：分析一下刚才的结果、帮我总结一下之前的数据

4. sql_and_analysis - 查询【信用卡系统数据库】后进行深度分析，无需外部数据
   示例：分析各活动刷卡金发放情况、找出返现异常的记录并分析原因

5. web_search - 需要从互联网获取【外部/行业/市场】信息，答案不在公司数据库中
   示例：银行信用卡平均返现比例是多少、2025 年信用卡优惠活动趋势、主流银行刷卡返现规则、行业平均返现水平

6. search_and_sql - 需要【同时】查询公司内部数据库 AND 联网搜索行业外部数据，进行内外对比
   示例：我们平台的刷卡金使用率和行业平均水平相比怎么样、内部餐饮类交易趋势和行业消费趋势是否一致、我们的活动返现力度和主流银行相比如何

【关键判断规则】
- 问题中出现"行业"、"市场"、"全国"、"社会平均"、"信用卡行业"等词 → 优先考虑 web_search
- 问题同时出现"我们平台/内部/本行"和"行业/市场/对比" → 选 search_and_sql
- 问题只涉及内部业务数据库 → 选 sql_only 或 sql_and_analysis
...
def get_search_and_sql_prompt(question: str, search_results: str, sql_results: str) -> str:
    ...
    Returns:
        联合分析提示词
    """
    return f"""你是一个信用卡业务数据分析专家，需要将行业外部数据（来自联网搜索）与内部业务数据（来自数据库）进行对比分析。
...
2. **对比分析**：内部业务数据与行业数据的差距或优势
3. **亮点与问题**：当前刷卡金活动或用户行为的特点
4. **建议**：基于对比结果给出可操作的建议
