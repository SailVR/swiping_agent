...
信用卡刷卡金智能数据分析 Agent 系统。项目面向信用卡刷卡金业务数据分析场景，基于 LangGraph 的"一主三从"架构，支持自然语言查数、NL2SQL 自动纠错、ECharts 数据可视化、Tavily 联网搜索、长短期记忆和 Excel/PDF 导出。

**技术栈**: LangGraph, LangChain, DashScope（通义千问 via OpenAI 兼容接口）, Tavily, MCP 协议, SQLite, Flask, ECharts, openpyxl, fpdf2, Rich。
...
- **前端 ↔ 后端**: HTTP REST API（`/api/login`, `/api/query`, `/api/query_stream`, `/api/export`, `/api/health`）+ SSE 流（`/api/query_stream`）
...
- **导出**: `/api/export` 从 `master_agent.session_data[thread_id]` 读取最近一次查询结果，调用 `export.py` 生成 Excel 或 PDF 文件
...
- 不要提交 `.env`、`__pycache__/`、`.DS_Store`、`logs/`、本地 SQLite 数据库和导出产物。
- 若修改 `agents/sql_agent.py` 或 `mcp_sql_server.py`，必须同时确认 `sql_validator.py` 的只读校验仍在 SQL 生成端和执行端生效。
- 若修改意图标签或路由逻辑，需要同步更新 `prompts.py`、`agents/master_agent.py`、前端意图展示和 README。
- 若修改数据库 schema，需要同步更新 `data/init_db.py`、NL2SQL few-shot 示例、README schema 描述和相关测试问题。
- `app.py` 中的 `user_systems` 是进程内内存状态，适合本地 demo；多进程或生产部署前需要换成外部会话存储。
- `.env` 只用于本地开发，文档中展示 API Key 时必须使用占位符。
