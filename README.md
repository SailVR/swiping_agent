...
> 基于 LangGraph 的信用卡刷卡金业务分析系统，采用**一主三从**多智能体架构，支持自然语言查数、NL2SQL 自动纠错、数据分析可视化、联网搜索、长短期记忆和 Excel/PDF 导出。

---

## 项目定位

本项目面向信用卡刷卡金业务数据分析场景，将用户的自然语言问题自动路由到不同 Agent：简单问题由主智能体直接回答，数据查询问题走 NL2SQL + SQLite 查询，分析类问题基于查询结果生成洞察和 ECharts 图表，外部信息类问题调用 Tavily 联网搜索。

系统适合用于信用卡活动效果分析、刷卡金发放/核销情况查询、用户分层分析、商户类型交易分布分析，以及内部数据与行业信息的联合对比。
...
- **一主三从多智能体**：意图识别、NL2SQL、数据分析、联网搜索
- **双层记忆系统**：短期 LLM 压缩 + 长期 SQLite 持久化
- **Web 前端**：Markdown 渲染、ECharts 图表、SSE 流式打字机效果
- **联网搜索**：Tavily 集成、搜索+SQL 联合对比、来源 URL 展示
- **数据导出**：Excel（带样式）+ PDF（自动中文检测）
- **SQL 安全校验**：白名单只读校验 + 21 个测试用例
- **轮转日志**：5MB/文件、3 备份、自动抑制 httpx/openai 噪讯
