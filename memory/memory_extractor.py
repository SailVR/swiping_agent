...
请提取以下类型的用户偏好：
1. favorite_department: 用户经常询问或关注的信用卡相关业务
2. query_focus: 用户的查询重点（如：刷卡金明细、资金统计、数据指标、异常核查等）
3. display_preference: 用户偏好的数据展示方式
4. common_topics: 用户常问的话题

以JSON格式返回（只返回JSON，不要其他文字）：
{{
    "favorite_department": "刷卡金明细",
    "query_focus": "商户名称",
    ...
}}
